...
const Section: React.FC<SectionProps> = ({ section, minimized, fullscreen, activeSection, handleSectionClick, handleState, children }) => {
  ...
  return (
    <section
      className={`${minimized ? 'h-[40px] w-[300px] bg-red-300' : 'bg-red-600'} relative flex-1 bg-[#262626] text-[#a1a1a1] rounded-[10px] box-border group ${activeSection === section ? 'border-green-500' : ''}`}
    ...
    >
      ...
      <h1 className={`h-[40px] bg-[#333333] rounded-t-[10px] inset-0 border-[1px] border-outset ${activeSection === section ? 'border-[#5d5d5d]' : 'border-transparent'} flex items-center justify-between px-[5px] relative`}>
