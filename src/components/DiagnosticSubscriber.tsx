    ...
    useEffect(() => {
        if (typeof EventSource !== 'undefined') {
            ...
            source.addEventListener('PUT', (e: MessageEvent) => {
                ...
                const formattedLog = log.status.diagnosticStatuses.map((status) => {
                    const keyValueStrings = status.values.keyValues.map(
                        (kv) => `${kv.key}: ${kv.value}`
                    ).join(', ');
    ...
    return (
        <div className="bg-red-300/20 shadow-md rounded-lg mb-4">
            <header className="p-4">
                <h5 id="panel_label" className="text-lg text-green-400 font-semibold m-0">Example DiagnosticArray data from /diagnostic topic published by example node in diagnotic_updater pkg </h5>
            </header>
            <div className="p-4" style={{ width: '100%', height: '48em', overflowY: 'auto' }}>
